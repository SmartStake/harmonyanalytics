...
def audit(conn, app, event, url, method, startTime, poolId=0, **kwargs):
    ...
    auditSimple(conn, app, startTime, url, method, sourceIp, poolId, urlParams)


def auditSimple(conn, app, startTime, url, method, sourceIp=None, poolId=None, urlParams=None):
...
def auditBot(conn, chatId, url, startTime, urlParams=None):
    ...
    conn.cursor().execute(sql, (chatId, url, "HarmonyBot", "GET", None,
        urlParams, responseTime))
    conn.commit()
    # logger.info("end of audit")
