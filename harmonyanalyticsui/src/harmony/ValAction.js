...
export default function ValAction(data) {
  ...
  const showStats = () => {
    document.location="/valstats/" + data.hPoolId;
  }

  const showDelegates = () => {
    document.location="/delegates/" + data.hPoolId;
  }

  const showEvents = () => {
    document.location="/events/" + data.hPoolId;
  }

  const showKeys = () => {
    document.location="/keys/" + data.hPoolId;
  }
  ...
  return (
    <div className={classes.root}>
      <SpeedDial ariaLabel="SpeedDial tooltip example"
        ...
        direction={direction}>
          <SpeedDialAction key="Stats" icon={<AssessmentIcon />} tooltipOpen tooltipTitle="Stats" onClick={showStats}/>
          <SpeedDialAction key="Delegates" icon={<PeopleIcon />} tooltipOpen tooltipTitle="Delegates" onClick={showDelegates}/>
          <SpeedDialAction key="Events" icon={<EventAvailableIcon />} tooltipOpen tooltipTitle="Events" onClick={showEvents}/>
          <SpeedDialAction key="BLS Key Performance" icon={<VpnKeyIcon />} tooltipOpen tooltipTitle="Keys" onClick={showKeys}/>
      </SpeedDial>
    </div>
  );
}
